...
from classification import load_classification_data, train_classification_model
...
dataloaders, num_classes, num_subclasses, class_to_idx, subclass_to_idx = load_classification_data()

idx_to_class = {v: k for k, v in class_to_idx.items()}
idx_to_subclass = {v: k for k, v in subclass_to_idx.items()}

print(idx_to_class)
print(idx_to_subclass)

# Initialize our classifier model with the number of output classes equal to num_classes

model = Classifier(num_classes, num_subclasses) # this will load the small model
